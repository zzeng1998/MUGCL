...
from MeGCN_cse import BertForMeGCN,BertForGCN
...
def load_fold_data(str):
    ...
    train_path = os.path.join("/home/zz/MGFND/" + '/Dataset/wechat/'+'/'+str+'/train.label.txt')
    test_path = os.path.join("/home/zz/MGFND/" + '/Dataset/wechat/'+'/'+str+'/test.label.txt')
...
def image_reshape(img_name):
    file = '/home/zz/MGFND/Dataset/wechat/image/'
...
def custom_collate(batch):
    ...
    batch_dict = {
        ...
        'id': [item[1].id.unsqueeze(0) for item in batch],
...
class GraphDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return Data(x_input_ids=torch.LongTensor([data['input_ids']]),
                    ...
                    y = torch.LongTensor([int(data['y'])])), Data(image = torch.tensor(image),id = torch.tensor(int(id)))
...
def train_fold(train_fold,test_fold):
    ...
    for epoch_ids in range(num_train_epochs):
            ...
            #all_feature = torch.zeros((1058+1059, 768)).to(device)
            all_feature = torch.empty(0, 768).to(device)
            label = torch.empty(0, ).to(device)
            id_index = torch.empty(0, ).to(device)
            for step, batch in enumerate(tqdm(train_loader, desc="Iteration")):
                
                input_ids, input_mask, segment_ids,edge,image,id, label_ids = batch["x_input_ids"],batch["x_input_mask"],batch["x_segment_ids"],batch["edge_index"],batch["image"],batch["id"],batch["y"]
                ...
                id = torch.concat(id, dim=0).to(device)
                edge = torch.concat(edge,dim=1).to(device)
                # batch = tuple(t.to(device) for t in batch)
                # input_ids, input_mask_tweet, segment_ids, label_ids = batch

                loss_g, image_out,pooled_output,_ = model(input_ids, segment_ids, input_mask, edge, image, label_ids)
...
output_dir = "/home/zz/MGFND/results/megcn/wechat"
bert_model = "chinese-bert-wwm"
num_labels = 2
